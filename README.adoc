...
Acquiring the validation data from TeaStore is a more complex process involving downloading kieker logs, transforming them and storing into an SQLite database. The code for this is currently not available on GitHub but is a private repository on JetBrains Space. Access is granted for reviewers after requesting it from the main author of the paper.
